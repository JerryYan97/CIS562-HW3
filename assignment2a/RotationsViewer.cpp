...
mMode(DEMO1), mXAngle(50), mYAngle(0), mZAngle(0), mRotOrder(XYZ)
{
	...
	vec3 col1(0.5f, 0.5f, 0.5f);
	vec3 col2(1.f, 1.f, 0.f);
	vec3 col3(0.f, 0.f, 1.f);
	vec3 col4(1.f, 0.f, 0.f);
	mColorSpline.appendKey(0, col1);
	mColorSpline.appendKey(3, col2);
	mColorSpline.appendKey(6, col3);
	mColorSpline.appendKey(9, col4);
	mColorSpline.setInterpolationType(ASplineVec3::CUBIC_HERMITE);

	vec3 pos1(-1.75f, -1.75f, 0.f);
	vec3 pos2( 1.75f, -1.75f, 0.f);
	vec3 pos3(-1.75f,  1.75f, 0.f);
	vec3 pos4( 1.75f,  1.75f, 0.f);
	mPositionSpline.appendKey(pos1);
	mPositionSpline.appendKey(pos2);
	mPositionSpline.appendKey(pos3);
	mPositionSpline.appendKey(pos4);
	mPositionSpline.appendKey(pos1);
	mPositionSpline.setInterpolationType(ASplineVec3::CUBIC_HERMITE);
...
// The euler here are defined to be transformed in the sequence of x -> y -> z ?
void RotationsViewer::drawDemo1()
{
	...
	//pushEulerRotation(YXZ, vec3(0, 90, 0));
    glutSolidTeapot(0.5);
    glPopMatrix();
...
void RotationsViewer::drawDemo2()
{
	...
	vec3 col = mColorSpline.getValue(time);
	vec3 pos = mPositionSpline.getValue(time);
