...
bool mat3::ToEulerAngles(RotOrder order, vec3& angleRad) const
{
	...
	switch (order)
	{
		...
		case ZXY:
			...
			if (angleRad[VX] > -M_PI_2 + EPSILON)// The return value stays at the range of [-pi/2, pi/2]
			{   
		...
		case YXZ:
			...
			if (angleRad[VX] > -M_PI_2 + EPSILON)// The return value stays at the range of [-pi/2, pi/2]
			{
				...
				else
				{
					// WARNING.  Not a unique solution.
					angleRad[VY] = 0.0f;
					angleRad[VZ] = 0.0f; // It may needs to be revised
					result = false;
				}
			}
			else
...
void quat::FromRotation (const mat3& rot)
{
	...
	mQ[VW] = sqrt(rot[0][0] + rot[1][1] + rot[2][2] + 1) / 2;// q's s
...
void quat::FromAxisAngle (const vec3& axis, double angleRad)
{
	...
	mQ[VW] = 0.0; mQ[VX] = 1.0; mQ[VY] = 0.0;  mQ[VZ] = 0.0;
	float cosHalfAngle = cos(angleRad / 2);
	float sinHalfAngle = sin(angleRad / 2);
	mQ[VW] = cosHalfAngle;
	vec3 u = axis;
	u *= sinHalfAngle;
	mQ[VX] = u[0];
